...
contract Vol {
   ...
   // @notice Funtion creates the first epoch
    function init() internal {
        epochs.push(Epoch(block.timestamp, 0, 0, 0, 0, 0, Status.Active));
    }
            ...
            if (userPositions[_epochIndex][i].user == msg.sender) {
                if (userPositions[_epochIndex][i].volPrediction == epoch.realizedVol) {
                    if (userPositions[_epochIndex][i].claimedReward == false) {
                        uint256 payoff = userPositions[_epochIndex][i].capitalProvided * epoch.payOffPerDollar;
    ...
    function finalizeEpoch() external {
        ...
        epoch.realizedVol = epoch.startPrice > epoch.finalPrice
        ...
        epoch.status = Status.Ended;
        epochs.push(Epoch(epoch.startTime + timeInterval, 0, 0, 0, 0, 0, Status.Active));
    }
